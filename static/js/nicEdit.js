 ...
 * 
 * Exytab translates ( http://exy.com.ua/ ) and add "blockquote" button
 */
...
var nicLinkButton = nicEditorAdvancedButton.extend({
    addPane: function () {
        ...
        this.addForm({
            "": {type: "title", txt: "Добавить ссылку"},
            href: {type: "text", txt: "URL", value: "http://", style: {width: "150px"}},
            title: {type: "text", txt: "Заголовок"},
            target: {
                type: "select",
                txt: "Открыть в",
                options: {"": "Текущем окне", _blank: "Новом окне"},
                style: {width: "100px"}
            }
        }, this.ln)
    }, submit: function (C) {
        ...
        if (A == "http://" || A == "") {
            alert("Вы должны добавить URL, что бы создать ссылку");
            return false
        }
        this.removePane();
        if (!this.ln) {
...
var nicImageButton = nicEditorAdvancedButton.extend({
    addPane: function () {
        ...
        this.addForm({
            "": {type: "title", txt: "Добавить изображение"},
            src: {type: "text", txt: "URL", value: "http://", style: {width: "150px"}},
            alt: {type: "text", txt: "Alt текст", style: {width: "100px"}},
            align: {type: "select", txt: "Выровнять", options: {none: "По умолчанию", left: "Слева", right: "Справа"}}
        }, this.im)
    }, submit: function (B) {
        var C = this.inputs.src.value;
        if (C == "" || C == "http://") {
            alert("Вы должны добавить URL что бы вставить изображение");
            return false
        }
        this.removePane();
        if (!this.im) {
...
var nicUploadButton = nicEditorAdvancedButton.extend({
    nicURI: "http://files.nicedit.com/", addPane: function () {
        this.im = this.ne.selectedInstance.selElm().parentTag("IMG");
        ...
        A.write('<div style="font-size: 14px; font-weight: bold; padding-top: 5px;">Загрузить изображение</div>');
        ...
        this.myStatus.setContent('<img src="http://files.nicedit.com/ajax-loader.gif" style="float: right; margin-right: 40px;" /><strong>Загрузка...</strong><br />Please wait');
        this.myForm.submit();
        ...
        if (C == false) {
            ...
            if (C.url) {
                ...
                if (this.im) {
                    this.im.setAttributes({src: C.url, width: (A && C.width) ? Math.min(A, C.width) : ""})
                }
                this.removePane()
            } else {
                if (C.error) {
                    this.requestInterval = false;
                    this.setProgress(100);
                    alert("Произошла ошибка при загрузке изображения (" + C.error + ").");
                    this.removePane()
                } else {
                    if (C.noprogress) {
                        this.progressWrapper.setStyle({display: "none"});
